#!/bin/bash
# This script must be run after resumetcp or resumeudp scripts have finished
# It receives a file (i.e. cde.txt or support.txt or ips.txt) 
# It creates files with open ports TCP /UDP from the file $file.resumenamep-[tcp|udp].gnmap 
...
cp ../$file.resumenmap-udp.gnmap .
    ...
    raw="_raw_"
        ...
        # Extract from the previous generated file, extract the IP of each line into $file_hostsTCP.csv
        ...
        line=1
        # Generates a file with the IP and TCP ports open: IP port1,port2,...,portn,
        ...
        # Generate the list of all TCP / UDP ports found open in the network
    ...
    echo -e "Top open ports: ${GREEN}head -10 "$file"__TCPportscount.csv${NC}"
    echo -e "All information was consolidated in files ${RED}"$file"_all_*${NC}";
    echo -e "#####################################################################"
