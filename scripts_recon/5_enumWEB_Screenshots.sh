...
#   Command eyewitness -f urls.txt -d screensdir --timeout 3
#####################################################################################################
RED='\033[0;31m'
GREEN='\033[0;32m'
NC='\033[0m' # No Color
...
for port in $(cat ./enumWEB/httpports.txt); do
    ...
    sed 's/^/http:\/\//' ./enumWEB/$filename | sed 's/$/:'$port'\//' >> $tempfilescreens;
done

#order the file and keep unique URLs
cat $tempfilescreens | sort -d |uniq > $filescreens
rm $tempfilescreens
numips=$(cat $filescreens | wc -l);
echo "####################################################################"
echo -e "        ${GREEN}TAKING SCREENSHOTS TO HTTP SERVERS ${NC}"
...
echo -e "        A total of ${RED} $numips ${NC} will be taken"
eyewitness -f $filescreens2 -d enumWEB/screenshotshttps --no-prompt --timeout 3
